...
from .dataloader import clear_linecache,line2input
from torch.utils.data import DataLoader
from .models import BiGRU,CpG_regression_layer,CpG_regression_layer_single,context_kinetics_predicter,CombinedModelv2,kinetics_layer
import torch
import random
...
def train(args):
    ...
    if args.model_type == "kinetics":
        model = BiGRU(num_inputs=2)
    elif args.model_type == "f": #kinetics+seq
        model = BiGRU(num_inputs=3)
    elif args.model_type in {'IPD','PW' ,"seq"}:
        model = BiGRU(num_inputs=1)
    elif args.model_type in {'contrast'}:
        model = BiGRU(num_inputs=3,output_size=3) #使用f,ipd,pw,进行三分类
    elif args.model_type == "pretrain":
        model=CpG_regression_layer()
    elif args.model_type == "pretrain2":
        # model=regression_layer()
        model=context_kinetics_predicter()

    elif args.model_type == "combined":

        # pretrained_model=CpG_regression_layer()
        # pretrained_model.load_state_dict( torch.load(args.seq_model, map_location=torch.device('cpu')))
        # feature_layer=pretrained_model.feature_layer
        # model=CombinedModel(pretained=feature_layer)

        # pretrained_model=kinetics_layer()
    ...
    if not args.balance:
        ...
        #     weight = [1.0 /i for i in class_counts]
        weight= torch.tensor(weight)
    weight=weight.float().to(device)
    best_model_path = os.path.join(model_dir, f'best_{args.model_type}.b{args.seq_len}.ckpt')
    # Loss and optimizer
    ...
    elif args.model_type in {"pretrain","pretrain2"}:
    ...
    for epoch in range(args.max_epoch_num):
        ...
        for i, sfeatures in enumerate(train_loader):
            ...
            if args.model_type=="kinetics":
            ...
            elif args.model_type in {"f","contrast"}:
                # 确定输入输出
                seq = line2input(sfeatures, 'seq')
                X = line2input(sfeatures, 'fkf')

                seq = seq.to(device)
                X = X.to(device)
                labels = labels.to(device)
                # 前向
                input = torch.cat([seq, X], dim=-1)
                outputs, logits = model(input)
                loss = criterion(outputs, labels)
            elif args.model_type in {"seq"}:
                # 确定输入输出
                seq = line2input(sfeatures, 'seq')
                seq = seq.to(device)
                labels = labels.to(device)
                ...
                outputs, logits = model(input)
                loss = criterion(outputs, labels)
            elif args.model_type == "IPD":
                # 确定输入输出
                # X = line2input(sfeatures, 'IPD')
                X = line2input(sfeatures, 'fk')
                X=X[:, :, :1]
                X = X.to(device)
                labels = labels.to(device)
                ...
                if args.model_type in {"pretrain2"}:
                    X = line2input(sfeatures, 'seq41')
                else:
                ...
                # 前向
                if args.model_type in {"pretrain2"}:
                    predIPD, predPW = model(X)
                    predIPD, predPW=predIPD.squeeze(-1).float(), predPW.squeeze(-1).float()
                else:
                ...
                if args.pseudo:
                    ...
                    X_pseudo=generate_pseudo_negative(seq,regressor_model=context_kinetics_model)
                    # 然后你可以把 pseudo_neg 作为 X 继续送入你的分类模型
                    labels_pseudo = torch.zeros(seq.size(0), dtype=torch.long,device=labels.device)

                    seq = torch.cat([seq, seq], dim=0)  # (2B, L, V)
                    X = torch.cat([X, X_pseudo], dim=0)  # (2B, L, 2)
                    labels= torch.cat([labels, labels_pseudo], dim=0)  # (2B,)
                    ...
                    if args.model_type in {"kinetics","combined","f","IPD","PW","contrast","seq"}:
                        ...
                        for vi, vsfeatures in enumerate(valid_loader):
                                ...
                                # Forward pass
                                    ...
                                    # 确定输入输出
                                    seq = line2input(vsfeatures, 'seq')
                                    seq = seq.to(device)
                                    vlabels = vlabels.to(device)
                                    # 前向
                                    input = seq
                                    voutputs, vlogits = model(input)
                                    ...
                                    vlabels = vlabels.to(device)
                                    # 前向
                                    vinput=torch.cat([seq,X],dim=-1)
                                    voutputs, vlogits = model(vinput)
                                else:
                                    raise  Exception
                        ...
                        if args.model_type not in {'contrast'}:
                            ...
                            v_auc =0
                            v_precision = metrics.precision_score(vlabels_total, vpredicted_total, average="macro")
                            v_recall = metrics.recall_score(vlabels_total, vpredicted_total, average="macro")
                            ...
                            if len(curr_best_accuracy_epoches) > 0 and curr_best_accuracy_epoch > \
                                ...
                                torch.save(model.state_dict(),
                                           ...
                                           '.betterthanlast.b{}_epoch{}.ckpt'.format(args.seq_len,
                                                                                     epoch + 1))
                        if v_auc > curr_best_auc:
                            curr_best_auc = v_auc
                            # 构造最佳模型保存路径
                            best_model_path = os.path.join(model_dir, f'best_{args.model_type}.b{args.seq_len}.ckpt')
                            # 保存当前最优模型（覆盖旧的）
                            torch.save(model.state_dict(), best_model_path)
                            LOGGER.info(f"Best model updated at epoch {epoch + 1}, saved to {best_model_path}")
...
def main():
    ...
    st_train.add_argument('--pseudo', action='store_true',
                          help="whether add pseudo negative")
