...
def _get_necessary_items_of_a_alignedsegment(readitem):
    ...
    try:
        ...
        try:
            tag_fi = readitem.get_tag("ip")
            tag_fp = readitem.get_tag("pw")
            tag_ri =readitem.get_tag("ip")
            tag_rp = readitem.get_tag("pw")
        except KeyError:
    ...
    try:
        ...
        try:
            tag_fn=readitem.get_tag("np")
            tag_rn =readitem.get_tag("np")
        except KeyError:
            tag_fn = tag_rn = 0
...
def _normalize_signals(signals, normalize_method="zscore"):
    ...
    elif normalize_method == 'ks': #kinetics tools style
        percentile = min(90, (1.0 - 1.0 / (signals.size - 1)) * 100)
        localPercentile = np.percentile(signals, percentile)
        capValue = max(4.0*np.median(signals).item(), localPercentile)
        signals = np.minimum(signals, capValue)
        sshift, sscale = 0, np.mean(signals)
    else:
        raise ValueError("")
    if sscale == 0.0:
...
def extract_features_from_double_strand_read(alignedsegment_tmp, motifs, holeids_e, holeids_ne, dnacontigs,
    ...
    seq_qual = np.array(fwd_qual, dtype=int) if fwd_qual and len(fwd_qual)> 0 else np.full(len(seq_seq), 0, dtype=np.int32)
    ...
    if len(ipdmean_fwd) != len(seq_seq) or len(pwmean_fwd) != len(seq_seq):
        ...
        print("read-{} has no/uncomplated fwd ipd/pw values".format(seq_name))
        return []
    # if len(ipdmean_rev) != len(seq_seq) or len(pwmean_rev) != len(seq_seq):
    #     print("read-{} has no/uncomplated rev ipd/pw values".format(seq_name))
    ...
    matched = np.array(q_to_r_poss[:-1] >= 0)  # -1 insertion , -2 deletion
    ipdmean_fwd = ipdmean_fwd[seq_start:seq_end][matched]
    pwmean_fwd = pwmean_fwd[seq_start:seq_end][matched]
    q_to_r_poss=q_to_r_poss[:-1][matched]  #全部是对齐的序列,应该是连续的
    ...
    for matchedloc in range(len(q_to_r_poss)):
        #matched loc是所有match序列中的排序
        loc=q_to_r_poss[matchedloc] #是ref中相对ref_start的偏移量
        if 10 <= loc < len(refseq) - 10:
            fkmer_seq = refseq[(loc - 10):(loc + 10 + 1)]
            fkmer_im = ipdmean_fwd[matchedloc:matchedloc+1]
            fkmer_pm = pwmean_fwd[matchedloc:matchedloc+1]
            ...
            if reverse:
                    chrom_pos = ref_end - 1 - loc
            else:
                    chrom_pos = loc + ref_start


            # feature_list.append([chrom, chrom_pos, strand, seq_name, loc,
            #                      fkmer_seq, npass_fwd, fkmer_im, ".", fkmer_pm, ".",
            #                      ".", ".",
            #                      ".", ".", ".", ".", ".", ".",
            #                      ".", ".",
            #                      args.methy_label])
            feature_list.append([chrom, chrom_pos, strand, fkmer_im,fkmer_pm])
    return feature_list
...
def _features_to_str(features):
    chrom, chrom_pos, strand, fkmer_im, fkmer_pm=features
    fkmer_im_str = ",".join([str(x) for x in fkmer_im])
    fkmer_pm_str = ",".join([str(x) for x in fkmer_pm])
    return "\t".join([chrom, str(chrom_pos), strand, fkmer_im_str,fkmer_pm_str])
    ...
    # fkmer_qual_str = ",".join([str(x) for x in fkmer_qual]) if type(rkmer_isd) is not str else "."
    # fkmer_map_str = ",".join([str(x) for x in fkmer_map]) if type(fkmer_map) is not str else "."
    #
    # rkmer_im_str = ",".join([str(x) for x in rkmer_im]) if type(rkmer_isd) is not str else "."
    # rkmer_isd_str = ",".join([str(x) for x in rkmer_isd]) if type(rkmer_isd) is not str else "."
    # rkmer_pm_str = ",".join([str(x) for x in rkmer_pm]) if type(rkmer_isd) is not str else "."
    # rkmer_psd_str = ",".join([str(x) for x in rkmer_psd]) if type(rkmer_psd) is not str else "."
    # rkmer_qual_str = ",".join([str(x) for x in rkmer_qual]) if type(rkmer_isd) is not str else "."
...
def main():
    ...
    p_extract.add_argument("--norm", action="store", type=str, 
                           choices=["zscore", "min-mean", "min-max", "mad", "none","ks"],
